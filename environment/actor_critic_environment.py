...
class ActorCriticEnvironment:
    ...
    def train(self, agent, optimizer, num_rollouts, train_steps=1, episodes_per_rollout=1, eval_episodes=0, td_steps=1, plot=False):
        episode_reward = []
        evaluation_episode = []
        evaluation_reward = []
        ...
        for roll in range(num_rollouts):
            ...
            for _ in range(train_steps):
                loss = agent.loss(self.buffer.all_episodes(), trajectory_length=td_steps)
                optimizer.zero_grad()
                loss.backward()
